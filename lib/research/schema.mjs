...
export const reportSchema = obj({
  ...
  companyInfo:arr(obj({key:choice('sector','industry','ceo','website','headquarters','founded','ipoDate','identifiers','cfi'),value:nullable('string'),...evidence})),
...
const legacySchema=structuredClone(reportSchema);legacySchema.properties.schemaVersion.enum=['1.0'];delete legacySchema.properties.companyInfo;legacySchema.required=legacySchema.required.filter(k=>k!=='companyInfo');
const schemaFor=report=>report?.schemaVersion==='1.0'?legacySchema:reportSchema;
...
export function validateReport(report){
  ...
  const infoKeys=new Set();for(const row of report.companyInfo||[]){if(infoKeys.has(row.key))throw new Error('Duplicate company info key');infoKeys.add(row.key);if(row.value!==null&&(!row.value.trim()||!row.sourceIds.length||!row.locator.trim()))throw new Error('Company info requires value and evidence');if(row.sourceIds.some(id=>!ids.has(id)))throw new Error('Company info: unresolved source reference');if(row.key==='website'&&row.value&&!safeSourceUrl(row.value))throw new Error('Unsafe company website');}
...
// Recover the narrow, known metric error without accepting an unsupported number.
// Imports remain strict; generated reports omit the affected rows with a visible gap.
export function validateGeneratedReport(input){
  check(schemaFor(input),input);
  const report=structuredClone(input);
  const omitted=report.metrics.filter(m=>m.kind==='calculated'&&!m.calculation.trim());
  if(omitted.length){
    report.metrics=report.metrics.filter(m=>!omitted.includes(m));
    report.coverage.status='partial';
    report.coverage.limitations.push(`Omitted calculated metrics without formulas: ${omitted.map(m=>m.label+' ('+m.period+')').join(', ')}. No values were inferred or repaired.`);
  }
  const entities=new Map(report.subsidiaries.map(s=>[s.id,s]));
  const broken=[];
  for(const entity of report.subsidiaries){
    const seen=new Set([entity.id]);let parent=entity.parentId;
    while(parent){if(seen.has(parent)||!entities.has(parent)){broken.push(entity);break;}seen.add(parent);parent=entities.get(parent).parentId;}
  }
  for(const entity of broken){
    entity.parentId=null;
    entity.disclosureStatus+=' Parent relationship unresolved: invalid generated hierarchy link was removed; no ownership relationship was inferred.';
  }
  if(broken.length){report.coverage.status='partial';report.coverage.limitations.push(`Unresolved parent relationships: ${broken.map(s=>s.name).join(', ')}. Entity descriptions retained; invalid links removed.`);}
  if(report.schemaVersion==='1.1'){
    const keys=['sector','industry','ceo','website','headquarters','founded','ipoDate','identifiers','cfi'];
    for(const key of keys)if(!report.companyInfo.some(f=>f.key===key))report.companyInfo.push({key,value:null,sourceIds:[],locator:'Not supplied by this research run.'});
    const missing=report.companyInfo.filter(f=>!f.value).map(f=>f.key);
    if(missing.length){report.coverage.status='partial';report.coverage.limitations.push('Company facts not established: '+missing.join(', ')+'.');}
    for(const type of ['annual','quarterly'])if(!report.metrics.some(m=>m.periodType===type&&m.value!==null)){
      report.coverage.status='partial';report.coverage.limitations.push(`No numeric ${type} financial data was returned. Research is incomplete for this period type.`);
    }
  }
