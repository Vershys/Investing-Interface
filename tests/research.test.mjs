...
test('App Server handshake, structured output, final response and cancellation use the protocol',async()=>{
 ...
 proc.stdin.on('data',b=>{for(const line of b.toString().trim().split('\n')){const m=JSON.parse(line);sent.push(m);if(m.id===undefined)continue;let result={};if(m.method==='thread/start')result={thread:{id:'thr_test'}};if(m.method==='turn/start')result={turn:{id:'turn_test'}};proc.stdout.write(JSON.stringify({id:m.id,result})+'\n');if(m.method==='turn/start'&&complete)queueMicrotask(()=>{proc.stdout.write(JSON.stringify({method:'item/completed',params:{threadId:'thr_test',item:{type:'agentMessage',text:'{"ok":true}'}}})+'\n');proc.stdout.write(JSON.stringify({method:'turn/completed',params:{threadId:'thr_test',turn:{status:'completed'}}})+'\n');});}});
 const adapter=new CodexAdapter({cwd:tmpdir(),spawnProcess:()=>proc});try{const result=await adapter.run({prompt:'test',schema:{type:'object'}});assert.equal(result.text,'{"ok":true}');assert.deepEqual(sent.slice(0,2).map(m=>m.method),['initialize','initialized']);assert.equal(sent.find(m=>m.method==='turn/start').params.outputSchema.type,'object');complete=false;const controller=new AbortController();const pending=adapter.run({prompt:'cancel',signal:controller.signal});setTimeout(()=>controller.abort(),10);await assert.rejects(pending,/cancelled/);assert(sent.some(m=>m.method==='turn/interrupt'));}finally{adapter.close();}
});
